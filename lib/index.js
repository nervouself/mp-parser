...
class MpParser {
  ...
  parseDirs() {
    ...
    const files = utils.getDirFiles(this._conf.from, this._conf.needParseExts);
  ...
  copyDirs() {
    ...
    const files = utils.getDirFiles(this._conf.from, this._conf.needCopyExts);
    if (files && files.length) {
      for (let file of files) {
        this.copyFile(file);
      }
    } else {
      utils.log.warn(`Files "${this._conf.needCopyExts.join(',')}" has 0 file that needs copy.`);
    }
  }
