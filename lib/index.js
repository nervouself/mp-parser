...
class MpParser {
  ...
  parseApp() {
    ...
    utils.createFile(outPath, utils.parseFile(inpath, this._conf.browsers), this._conf.exts);
  }
    ...
    for (let dir of this._conf.needParseDirs) {
      ...
      if (files && files.length) {
        for (let file of files) {
          ...
          utils.createFile(path.join(outPath, relativePath), utils.parseFile(file, this._conf.browsers), this._conf.exts);
        }
      } else {
  ...
  parseFile(file) {
    ...
    utils.createFile(path.join(this._conf.to, relativePath), utils.parseFile(file, this._conf.browsers), this._conf.exts);
  }
